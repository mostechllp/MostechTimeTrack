...
const StaffManagement = () => {
  ...
  const fetchStaff = async () => {
    try {
      // Always fetch all staff to know counts
      const { data } = await axiosInstance.get(`/admin/staff?includeDeleted=true`);
      setStaff(data);
    } catch (error) {
  ...
  const getFilteredStaff = () => {
    if (activeTab === "active") {
      return staff.filter(member => !member.isDeleted);
    } else {
      return staff.filter(member => member.isDeleted);
    }
  };

  const activeCount = staff.filter(m => !m.isDeleted).length;
  const deletedCount = staff.filter(m => m.isDeleted).length;
  ...
  const onUpdate = async (data) => {
    ...
    setIsSubmitting(true);
    try {
      const response = await axiosInstance.put(`/admin/staff/${editingStaff._id}`, data);
  ...
  const handleEdit = (staffMember) => {
    ...
    setValue('joiningDate', staffMember.joiningDate?.split('T')[0] || new Date().toISOString().split('T')[0]);
  ...
  const confirmDelete = async () => {
    try {
      const response = await axiosInstance.delete(`/admin/staff/${selectedStaff._id}`);
  ...
  const confirmRestore = async () => {
    try {
      const response = await axiosInstance.put(`/admin/staff/${selectedStaff._id}/restore`);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        {showAddForm && (
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 lg:p-8 mb-6 sm:mb-8 animate-slideDown">
            <h2 className="text-lg sm:text-xl font-bold mb-4 sm:mb-6" style={{ color: "#020c4c" }}>
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredStaff.length === 0 ? (
                  ...
                  filteredStaff.map((member) => (
                    <tr key={member._id} className={`hover:bg-gray-50 ${member.isDeleted ? 'bg-gray-50' : ''}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                        {new Date(member.joiningDate || member.createdAt).toLocaleDateString()}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {member.isDeleted ? (
          ...
          <div className="md:hidden">
            {filteredStaff.map((member) => (
              <div key={member._id} className={`p-4 border-b border-gray-200 ${member.isDeleted ? 'bg-gray-50' : ''}`}>
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to delete ${selectedStaff?.firstName} ${selectedStaff?.lastName}? This action can be reversed later.`}
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to restore ${selectedStaff?.firstName} ${selectedStaff?.lastName}?`}
