...
const ReportModal = ({ 
  ...
  const hasUnsavedChanges = () => {
    ...
    if (existingReport && isEditingMode) {
      return formData.workDone !== (existingReport.workDone || '') ||
             ...
             formData.challenges !== (existingReport.challenges || '') ||
             formData.tomorrowPlan !== (existingReport.tomorrowPlan || '');
    }
    return false;
  };

  const hasUnsavedRemark = () => {
    return remarks.trim() !== '';
  };
  ...
  const handleCloseAttempt = () => {
    ...
    } else if (isAddingRemark && hasUnsavedRemark()) {
      setShowRemarkCancelConfirm(true);
    } else {
      onClose();
    }
  };
  ...
  return (
    <>
          ...
          <Dialog.Panel className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 py-5">
              ...
              {isAddingRemark ? (
                ...
                <form ref={formRef} onSubmit={(e) => { e.preventDefault(); handleSubmitClick(); }} className="space-y-5">
                  <FormField
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to ${existingReport ? 'update' : 'submit'} this report?`}
        confirmText="Yes, Submit"
        cancelText="No, Keep Editing"
        confirmButtonColor="#020c4c"
      />

      {/* Remark Cancel Confirmation Modal */}
      <ConfirmModal
        isOpen={showRemarkCancelConfirm}
        onClose={() => setShowRemarkCancelConfirm(false)}
        onConfirm={() => {
          setShowRemarkCancelConfirm(false);
          setIsAddingRemark(false);
          setRemarks('');
        }}
        title="Discard Remark?"
        message="You have unsaved remark. Are you sure you want to discard it?"
        confirmText="Yes, Discard"
        cancelText="No, Keep Editing"
        confirmButtonColor="#dc2626"
      />
